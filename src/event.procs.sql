DROP FUNCTION IF EXISTS conreality.event_send(text, text, text) RESTRICT;

CREATE FUNCTION conreality.event_send(event_predicate text,
                                      event_subject text,
                                      event_object text) RETURNS bigint AS $$
  local query  = "INSERT INTO conreality.event (predicate, subject, object) VALUES ($1, $2::uuid, $3::uuid) RETURNING id";
